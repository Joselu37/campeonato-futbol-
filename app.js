...
function openPlayoffScoreModal(matchKey) {
  ...
  if (!matchObj) {
    alert('No se encontró este cruce.');
    return;
  }

  const ranking = calculateQualifiedTeamsRanking(appState.currentCategory);
  const qualifiedTeams = (ranking.qualifiedList || [])
    .filter(item => item.team)
    .map(item => ({ id: item.team.id, name: item.team.name, rankLabel: item.rankLabel }));

  const homeSelect = document.getElementById('modalPlayoffHomeTeamSelect');
  const awaySelect = document.getElementById('modalPlayoffAwayTeamSelect');
  const optionsHtml = '<option value="">-- Seleccionar equipo --</option>' + qualifiedTeams.map(t => `<option value="${t.id}">${t.name} (${t.rankLabel})</option>`).join('');
  if (homeSelect) { homeSelect.innerHTML = optionsHtml; homeSelect.value = matchObj.home || ''; }
  if (awaySelect) { awaySelect.innerHTML = optionsHtml; awaySelect.value = matchObj.away || ''; }
...
function savePlayoffSchedule() {
  ...
  if (matchObj) {
    const homeTeamId = document.getElementById('modalPlayoffHomeTeamSelect').value;
    const awayTeamId = document.getElementById('modalPlayoffAwayTeamSelect').value;
    if (homeTeamId && awayTeamId && homeTeamId === awayTeamId) {
      alert('El equipo Local y el Visitante no pueden ser el mismo equipo.');
      return;
    }
    matchObj.home = homeTeamId || matchObj.home;
    matchObj.away = awayTeamId || matchObj.away;
...
function savePlayoffScore() {
  ...
  if (matchObj) {
    ...
    if (!matchObj.home || !matchObj.away) {
      alert('Definí los dos equipos (Local y Visitante) antes de cargar un resultado.');
      return;
    }
